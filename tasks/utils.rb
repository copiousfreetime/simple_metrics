...
module Utils
  class << self
    ...
    def announcement
      ...
      { 
        :subject        => "#{cfg.name} #{SimpleMetrics::VERSION} Released",
        :title          => "#{cfg.name} version #{SimpleMetrics::VERSION} has been released.",
        :urls           => "#{cfg.homepage}",
        :description    => "#{cfg.description.rstrip}",
        :release_notes  => Utils.release_notes_from(cfg.history)[SimpleMetrics::VERSION].rstrip 
      }
    end
  end
end # << self
